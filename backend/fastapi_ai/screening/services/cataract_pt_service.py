...
class CataractPtService:
    ...
    # Severity weights for the continuous p_cataract score used by the fusion.
    # 0=normale, 1=legere, 2=moderee, 3=severe. The weighted formula keeps a
    # mild ("legere") cataract well below the asymmetry threshold (0.30) so it
    # does not artificially push p_finale into CRITICAL when combined with a
    # strong tabular signal. Severe still scores ~1.0 to fire the boost rules.
    _SEVERITY_WEIGHTS = np.asarray([0.0, 0.25, 0.65, 1.0], dtype=np.float32)
    ...
    def predict(self, image_bytes: bytes) -> CataractInferenceResult:
        ...
        # Severity-weighted p_cataract instead of raw sum of non-normal probs
        # (which would over-trigger the fusion for "legere" classifications).
        p_cataract = float(np.sum(probs * self._SEVERITY_WEIGHTS))
    ...
    def generate_gradcam(self, image_bytes: bytes) -> dict:
        ...
        prediction_label = CATARACT_CLASS_LABELS[prediction_idx]
        confidence = float(probs[prediction_idx])
        # Severity-weighted p_cataract (must stay consistent with predict()).
        p_cataract = float(np.sum(probs * self._SEVERITY_WEIGHTS))
