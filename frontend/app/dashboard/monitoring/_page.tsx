...
export default function MonitoringPage() {
  ...
  useEffect(() => {
    ...
    void listMedications(patientId)
      .then((payload) => {
        if (!cancelled) setMedications(payload.items)
      })
      .catch((error: unknown) => {
        if (!cancelled) setMedicationError(error instanceof Error ? error.message : 'Failed to load medications')
      })
      .finally(() => {
    ...
    void listLabResults(patientId)
      .then((payload) => {
        if (!cancelled) setLabResults(payload.items)
      })
      .catch((error: unknown) => {
        if (!cancelled) setMonitoringError(error instanceof Error ? error.message : 'Failed to load lab results')
      })
      .finally(() => {
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      {user?.role !== 'patient' && (
        <Card className="border-primary/25 bg-primary/5 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-semibold">{t('patientContext')}</CardTitle>
            <CardDescription>
              {patientId
                ? contextPatientName
                  ? t('patientContextWithName', { name: contextPatientName })
                  : t('patientContextNoName')
                : user?.role === 'doctor'
                  ? t('patientContextDoctor')
                  : t('patientContextCaregiver')}
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
            ...
            <p className="max-w-md leading-relaxed">
              {isDoctor ? t('pickPatientDoctor') : t('pickPatientCaregiver')}
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('medicationsDesc')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {!patientId && (
            <p className="text-sm text-muted-foreground">{t('choosePatientForMeds')}</p>
          )}

          {patientId && medicationLoading && (
            <p className="text-sm text-muted-foreground">{t('loadingMedications')}</p>
          )}
                  ...
                  <div className="medication-details">
                    ...
                    {medication.instructions && (
                      <div className="detail-item col-span-2 mt-1 italic text-[11px] text-muted-foreground border-t border-border/50 pt-1">
                        <span>{t('note')} {medication.instructions}</span>
                      </div>
                    )}
                  </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('labResultsDesc')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {patientId && labLoading && labResults.length === 0 && (
            <p className="text-sm text-muted-foreground">{t('loadingLabResults')}</p>
          )}

          {patientId && !labLoading && labResults.length === 0 && !monitoringError && (
            <p className="text-sm text-muted-foreground">{t('noLabResults')}</p>
          )}
                    ...
                    <div className="flex items-center gap-3 text-left w-full">
                      <div className="h-9 w-9 rounded-md bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden border">
                        {doc.preview_url && isImage(doc.mime_type) ? (
                          <img src={doc.preview_url} alt="" className="h-full w-full object-cover" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {new Date(doc.date).toLocaleDateString()} • {doc.results.length} {t('findings')}
                        </div>
                      </div>
                      {doc.preview_url && (
                        <span
      ...
      <Tabs defaultValue="alerts" className="w-full">
            ...
            <CardHeader>
              ...
              <CardDescription>
                {isDoctor
                  ? t('alertsDescDoctor')
                  : isCaregiver
                    ? t('alertsDescCaregiver')
                    : t('alertsDescPatient')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {monitoringLoading && <p className="text-sm text-muted-foreground">{t('loadingAlerts')}</p>}
              {!monitoringLoading && alerts.length === 0 && (
                <p className="text-sm text-muted-foreground">{t('noAlertsScope')}</p>
              )}
              {alerts.map((alert) => {
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                      <div className="flex gap-3 flex-1 min-w-0">
                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${iconWrapClass}`} aria-hidden>
                          <AlertStatusIcon className="h-5 w-5" strokeWidth={2} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="font-semibold">{alert.title}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{alert.patient_username}</p>
                          <p className="text-sm text-muted-foreground mt-2 whitespace-pre-wrap leading-relaxed">{alert.message}</p>
                        </div>
                      </div>
        ...
        <TabsContent value="progression" className="space-y-4">
          {isCaregiver ? (
            <Card>
              <CardHeader>
                <CardTitle>{t('progressionSummary')}</CardTitle>
                <CardDescription>{t('caregiverProgressionDesc')}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="rounded-lg border border-border bg-muted/30 p-4">
                  <p className="font-medium">{t('clinicianTrendsHidden')}</p>
                  <p className="text-sm text-muted-foreground mt-2">{t('caregiverProgressionInfo')}</p>
                </div>
              </CardContent>
            </Card>
          ) : (
            ...
            {selectedDoc?.url && (
              previewError ? (
                <div className="flex h-full w-full flex-col items-center justify-center gap-3 rounded-sm border border-border bg-background p-6 text-center text-sm text-muted-foreground">
                  ...
                  <p>{t('previewError')}</p>
                  <a href={selectedDoc.url} target="_blank" rel="noreferrer" className="text-primary underline">
                    {t('openInNewTab')}
                  </a>
                </div>
              ) : (
