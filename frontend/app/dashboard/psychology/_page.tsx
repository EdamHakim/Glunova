...
export default function PsychologyPage() {
  ...
  const voicePhaseLine = useMemo(() => {
    if (avatarPhase === 'listening') return t('voicePhaseListening')
    if (avatarPhase === 'thinking') return t('voicePhaseThinking')
    if (avatarPhase === 'speaking') return t('voicePhaseSpeaking')
    return t('voicePhaseIdle')
  }, [avatarPhase, t])
      ...
      <div className="relative min-h-[calc(100dvh-6rem)]">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
          <header className="mb-8 max-w-2xl space-y-3">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">{t('clinicalCompanion')}</p>
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Sanadi</h1>
            <p className="text-base leading-relaxed text-muted-foreground">{t('notPatientDesc')}</p>
          </header>
          <Card className="max-w-xl border-border/80 shadow-md shadow-black/5">
            <CardHeader>
              <CardTitle>{t('careTeamAccessTitle')}</CardTitle>
              <CardDescription>{t('careTeamAccessDesc')}</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-sm leading-relaxed text-muted-foreground">{t('careTeamAccessContent')}</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 sm:py-10">
          <header className="max-w-2xl space-y-3">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">{t('mentalWellness')}</p>
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Sanadi</h1>
            <p className="text-base leading-relaxed text-muted-foreground">{t('calmCompanion')}</p>
          </header>
            ...
            <Card className="overflow-hidden border-border/80 shadow-md shadow-black/5">
              <CardHeader className="space-y-0 border-b bg-linear-to-br from-muted/50 to-muted/10 pb-5 pt-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                  ...
                  <div className="min-w-0 space-y-2">
                    <CardTitle className="text-xl sm:text-2xl">{t('beginVisitTitle')}</CardTitle>
              ...
              <CardContent className="space-y-6 p-5 sm:p-6">
                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  <span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">{t('cbtGuidedSupport')}</span>
                  <span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">{t('safetyAwareRouting')}</span>
                  <span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">{t('cameraOptional')}</span>
                </div>
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">{t('sessionLanguage')}</p>
                  <LanguagePicker value={preferredSessionLang} onChange={setPreferredSessionLang} />
                  <p className="text-[0.7rem] leading-snug text-muted-foreground">{t('chooseBefore')}</p>
                </div>
                <div className="flex items-center justify-between gap-4 rounded-xl border border-border/60 bg-muted/15 px-4 py-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{t('startWithCamera')}</p>
                    <p className="text-[0.7rem] leading-snug text-muted-foreground">{t('cameraOptInDesc')}</p>
                  </div>
                ...
                <Button size="lg" className="w-full sm:w-auto sm:px-10" onClick={() => void beginSession()} disabled={startingSession}>
                  {startingSession ? t('starting') : t('beginSession')}
                </Button>
    ...
    >
      {(voiceRecording || micListening) && (
        ...
        >
          ...
          <span className="sr-only">{t('micActive')}</span>
        </div>
      )}
          ...
          <Sheet open={sessionToolsOpen} onOpenChange={setSessionToolsOpen}>
            ...
            >
              <div className="shrink-0 bg-linear-to-br from-primary/[0.09] via-muted/40 to-background px-6 pb-6 pt-10">
                ...
                <SheetTitle className="mt-1.5 text-left text-xl font-bold tracking-tight">{t('visitSettingsTitle')}</SheetTitle>
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-0 overflow-y-auto overscroll-contain px-4 pb-12 pt-1">
                <div className="flex gap-4 rounded-2xl border border-border/80 bg-card p-4 shadow-md shadow-black/[0.04]">
                  ...
                  <div className="min-w-0 flex-1 pt-0.5">
                    ...
                    <p className="mt-2 inline-flex items-center rounded-full border border-border/60 bg-muted/40 px-2.5 py-1 text-[0.7rem] font-medium text-muted-foreground">
                      {t('visitLabel', { ordinal: visitOrdinal })}
                ...
                <div className="rounded-2xl border border-border/80 bg-card/95 p-4 shadow-sm">
                  <p className="text-[0.65rem] font-semibold uppercase tracking-wide text-muted-foreground">{t('moodLabel')}</p>
                  <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t('moodDesc')}</p>
                ...
                <div className="rounded-2xl border border-border/80 bg-card/95 p-4 shadow-sm">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t('companionAvatarDesc')}</p>
                ...
                <div className="rounded-2xl border border-border/80 bg-card/95 p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="text-[0.65rem] font-semibold uppercase tracking-wide text-muted-foreground">{t('cameraLabel')}</p>
                      <p className="mt-1.5 text-sm font-semibold leading-snug">{t('cameraPreview')}</p>
                      <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                        {cameraOn ? t('cameraOnDesc') : t('cameraOffDesc')}
                      </p>
                    </div>
                    <Switch
                      checked={cameraOn}
                ...
                <div className="rounded-2xl border border-border/80 bg-muted/20 p-1 shadow-sm">
                  <Collapsible open={recognitionDebugOpen} onOpenChange={setRecognitionDebugOpen}>
                    ...
                    <CollapsibleContent className="px-3 pb-4 pt-0 data-[state=closed]:animate-none">
                      <p className="mb-2 text-xs leading-relaxed text-muted-foreground">{t('recognitionDebugDesc')}</p>
            ...
            <>
                ...
                <div className={cn('relative z-0 min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3', cameraOn && 'pt-20 sm:pt-3')}>
                  <div className="mx-auto flex min-h-full w-full max-w-lg flex-col items-center justify-center gap-4 pb-6">
                    ...
                    <div className="w-full max-w-[480px] shrink-0 overflow-hidden rounded-2xl border border-border/70 bg-card/95 p-4 shadow-sm">
                      <p className="mb-3 text-center text-xs font-medium tracking-wide text-muted-foreground">{t('yourVoice')}</p>
                      <SanadiVoiceWaveform
              ...
              <div className="relative z-20 shrink-0 border-t border-border/40 bg-[color:var(--sanadi-shell-bg)] px-3 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-12px_24px_-18px_rgb(15_23_42/0.12)] dark:shadow-[0_-12px_24px_-18px_rgb(0_0_0/0.45)]">
                ...
                <div className="mx-auto flex w-full max-w-lg items-stretch gap-2">
                  ...
                  >
                    {voiceRecording ? t('doneSpeaking') : t('tapToSpeak')}
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
            ...
            >
              {entry.kind === 'thought_record' ? (
                <div className="w-[min(42rem,90vw)] rounded-[1.25rem] border border-border bg-card/80 p-4 shadow-sm">
                  <p className="mb-3 text-sm font-medium">{t('thoughtRecord')}</p>
                  <div className="space-y-2">
                    <label className="block text-xs text-muted-foreground">{t('whatHappened')}</label>
                    <textarea
                    ...
                    />
                    <label className="block text-xs text-muted-foreground">{t('whatIThought')}</label>
                    <textarea
                      className="w-full rounded-xl border bg-background p-2 text-sm"
                    ...
                    />
                    <label className="block text-xs text-muted-foreground">{t('howItFelt')}</label>
                    <textarea
                      className="w-full rounded-xl border bg-background p-2 text-sm"
                    ...
                    />
                    <label className="block text-xs text-muted-foreground">{t('anotherWay')}</label>
                    <textarea
                      className="w-full rounded-xl border bg-background p-2 text-sm"
