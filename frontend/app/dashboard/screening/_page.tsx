...
import { Upload, Mic, AlertCircle, StopCircle, Eye } from 'lucide-react'
...
export default function ScreeningPage() {
  ...
  async function onTongueSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetchWithAuthRefresh(`${fastapi}/screening/tongue/infer`, {
        method: 'POST',
        credentials: 'include',
        body: payload,
      })
      ...
      const gradcamResponse = await fetchWithAuthRefresh(`${fastapi}/screening/tongue/gradcam`, {
        method: 'POST',
        credentials: 'include',
        body: gradcamPayload,
      })
  ...
  async function onVoiceSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetchWithAuthRefresh(`${fastapi}/screening/voice/infer`, {
        method: 'POST',
        credentials: 'include',
        body: payload,
      })
    ...
    >
      <div className="space-y-6 p-4 sm:p-6">
        <Dialog open={isResultModalOpen} onOpenChange={setIsResultModalOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('tongueDialogTitle')}</DialogTitle>
              <DialogDescription>
                {viewerName
                  ? t('tongueDialogDescWithName', { name: viewerName })
                  : t('tongueDialogDesc')}
              </DialogDescription>
            </DialogHeader>
            ...
            {tongueResult ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm text-muted-foreground">{t('predictionLabel')}</span>
                ...
                {tongueResult.heatmapBase64 ? (
                  <div className="rounded-lg border p-2">
                    <p className="text-sm text-muted-foreground mb-2">{t('gradCamTitle')}</p>
                    <img
            ...
            <DialogFooter>
              <Button onClick={() => setIsResultModalOpen(false)}>{t('closeBtn')}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isCataractModalOpen} onOpenChange={setIsCataractModalOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('cataractDialogTitle')}</DialogTitle>
              <DialogDescription>
                {viewerName
                  ? t('cataractDialogDescWithName', { name: viewerName })
                  : t('cataractDialogDesc')}
              </DialogDescription>
            </DialogHeader>
            ...
            {cataractResult ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm text-muted-foreground">{t('severityLabel')}</span>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm text-muted-foreground">{t('likelihoodOfCataract')}</span>
                  <span className="font-semibold">
                    {Math.round(cataractResult.p_cataract * 100)}%
                  </span>
                </div>
                <div className="rounded-lg border p-3">
                  <p className="text-sm text-muted-foreground mb-2">{t('breakdownTitle')}</p>
            ...
            <DialogFooter>
              <Button onClick={() => setIsCataractModalOpen(false)}>{t('closeBtn')}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <CardContent className="flex-1 flex flex-col justify-center gap-4">
              <form className="space-y-3" onSubmit={onVoiceSubmit}>
                ...
                <div className="rounded-md border bg-muted/40 p-3">
                  <p className="text-xs text-muted-foreground">{t('bestVoiceOption')}</p>
                </div>
                ...
                {voiceRecording ? (
                  <p className="text-xs font-medium text-health-danger">{t('recordingSeconds', { count: voiceRecordingSeconds })}</p>
                ) : null}
                {voiceFile ? (
                  <div className="space-y-2">
                    <p className="break-all text-xs text-muted-foreground">
                      {t('selectedLabel')} <span className="font-medium">{voiceFile.name}</span>
                    </p>
                  ...
                  <div className="aspect-video bg-muted rounded-lg flex items-center justify-center border-2 border-dashed border-border">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground">{t('uploadOrRecord')}</p>
                    </div>
                  </div>
                )}
                <Button type="submit" variant="outline" className="w-full" disabled={voiceLoading}>
                  {voiceLoading ? t('analyzingVoice') : t('runVoiceScreening')}
                </Button>
              </form>
            </CardContent>
          </Card>
          ...
          <Card className="flex flex-col">
            <CardHeader>
              ...
              <CardDescription>{t('tongueDesc')}</CardDescription>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col justify-center gap-4">
              <form className="space-y-3" onSubmit={onTongueSubmit}>
                ...
                <Button type="submit" className="w-full" disabled={loading}>
                  {loading ? t('analyzingLabel') : t('runTongueScreening')}
                </Button>
              </form>
            </CardContent>
          </Card>
          ...
          <Card className="flex flex-col">
            <CardHeader>
              ...
              <CardDescription>{t('cataractDesc')}</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-1 flex-col justify-center gap-4">
              <form className="space-y-3" onSubmit={onCataractSubmit}>
                ...
                <Button type="submit" className="w-full" disabled={eyeLoading}>
                  {eyeLoading ? t('analyzingLabel') : t('runCataractScreening')}
                </Button>
              </form>

              <Button variant="outline" className="w-full" asChild>
                <a href="/dashboard/screening/cataract">{t('advancedDetection')}</a>
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('aiAssessmentTitle')}</CardTitle>
            <CardDescription>{t('aiAssessmentDesc')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="p-4 border border-border rounded-lg">
                <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <span className="font-medium">{t('overallRiskScore')}</span>
                ...
                <div className="text-xs text-muted-foreground mt-2 space-y-1">
                  {tongueResult ? (
                    <p>
                      {t('tongueLabel')} <span className="font-medium text-foreground">{tongueResult.prediction_label}</span>
                    </p>
                  ) : null}
                  {voiceResult ? (
                    <p>
                      {t('voiceLabel')} <span className="font-medium text-foreground">{voiceResult.prediction_label}</span>
                    </p>
                  ) : null}
                  {cataractResult ? (
                    <p>
                      {t('cataractLabel')} <span className="font-medium text-foreground">{cataractResult.prediction_label}</span>{' '}
                      ({t('pCataractLabel')} {Math.round(cataractResult.p_cataract * 100)}%)
                    </p>
                  ) : null}
                  {!tongueResult && !voiceResult && !cataractResult ? (
                    <p>{t('submitToPopulate')}</p>
                  ) : null}
                </div>
              </div>
              ...
              {tongueResult ? (
                <div className="rounded-lg border p-3">
                  <p className="text-sm text-muted-foreground">{t('latestTongueResult')}</p>
              ...
              {voiceResult ? (
                <div className="rounded-lg border p-3">
                  <p className="text-sm text-muted-foreground">{t('latestVoiceResult')}</p>
                  <p className="font-semibold mt-1">
                    {voiceResult.prediction_label} ({Math.round(voiceResult.probability * 100)}%)
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('calibratedScore')} {Math.round(voiceResult.probability * 100)}% ({t('rawLabel')}{' '}
                    {Math.round(voiceResult.raw_probability * 100)}%)
                    {voiceResult.ood_flag ? ` · ${t('oodFlag')}` : ''}
                  </p>
                </div>
              ) : null}
              ...
              {cataractResult ? (
                <div className="rounded-lg border p-3">
                  <p className="text-sm text-muted-foreground">{t('latestCataractResult')}</p>
                  <p className="font-semibold mt-1">
                    {cataractResult.prediction_label} ({t('confidenceWith')} {Math.round(cataractResult.confidence * 100)}%)
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('pCataractLabel')} {Math.round(cataractResult.p_cataract * 100)}% · {t('severityIndex')} {cataractResult.prediction_index}
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-3 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">{t('voiceAnalysis')}</p>
                  <p className="font-semibold text-health-success">
                    {voiceResult ? `${Math.round(voiceResult.probability * 100)}% ${voiceResult.prediction_label}` : t('notAnalyzedYet')}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {voiceResult
                ...
                <div className="p-3 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">{t('tongueExamination')}</p>
                  <p className="font-semibold text-health-warning">
                    {tongueResult ? `${Math.round(tongueResult.probability * 100)}% ${tongueResult.prediction_label}` : t('notAnalyzedYet')}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {tongueResult ? t('tongueExamDesc') : t('uploadTongueAndRun')}
                  </p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">{t('eyeCataract')}</p>
                  <p
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {cataractResult ? t('mobilenetDesc') : t('uploadEyeAndRun')}
                  </p>
                  {cataractResult ? (
                    <Button variant="link" className="px-0 h-auto mt-1" onClick={() => setIsCataractModalOpen(true)}>
                      {t('openResult')}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('explainabilityDesc')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <p className="font-medium text-sm">{t('voiceFeatures')}</p>
              ...
              <div className="space-y-2">
                <p className="font-medium text-sm">{t('tongueImageLabel')}</p>
                {tongueResult?.heatmapBase64 ? (
                  ...
                  <div className="bg-muted rounded-lg p-3 aspect-square flex items-center justify-center">
                    <p className="text-xs text-center text-muted-foreground">
                      {t('runTongueForHeatmap')}
                    </p>
                  </div>
                )}
              </div>
              <div className="space-y-2">
                <p className="font-medium text-sm">{t('eyeLabel')}</p>
                {cataractResult?.heatmapBase64 ? (
                  <div className="bg-muted rounded-lg p-2 aspect-square">
                    <img
                      src={`data:image/jpeg;base64,${cataractResult.heatmapBase64}`}
                  ...
                  <div className="bg-muted rounded-lg p-3 aspect-square flex items-center justify-center">
                    <p className="text-xs text-center text-muted-foreground">
                      {t('runCataractForHeatmap')}
                    </p>
                  </div>
                )}
              </div>
              <div className="space-y-2 md:col-span-3">
                <p className="font-medium text-sm">{t('featureImportance')}</p>
